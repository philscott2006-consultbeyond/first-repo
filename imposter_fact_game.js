...
function main() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (providedNames.length > 0) {
      if (providedNames.length < MIN_PLAYERS) {
        alert(`Please add at least ${MIN_PLAYERS} players.`);
        return;
      }
      numPlayers = providedNames.length;
    } else {
      numPlayers = Number.parseInt(playerInput.value, 10);
    }

    if (!Number.isFinite(numPlayers) || numPlayers < MIN_PLAYERS) {
    ...
    currentIndex = 0;
    currentNames = providedNames.length > 0
      ? providedNames
